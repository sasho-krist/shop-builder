...
### 8b. Storefront polish + owner editing ✅

- **Section shell** — всяка секция рендерира съдържанието си през `SectionShell`
  (max-width `--sb-container` + странични gutters), еднакво за storefront и
  `PageCanvas`; продуктовите цени в секциите вече са с валутен символ.
- **Owner toolbar** — `SESSION_DOMAIN=.shop-builder.localhost` +
  `SESSION_COOKIE=sb_session` → влезлият собственик се разпознава и на
  поддомейна. `HandleInertiaRequests` дава `storefront.manage` (deep-link URL-и
  към админа) само когато `web` потребителят е член на магазина.
  `StorefrontOwnerBar` = плаваща лента (Edit home / Theme / Product / Catalog /
  Orders / Admin, скриваема). Всяка секция на home има hover „Edit <section>"
  който отваря `pages/{id}/edit?section=<blockId>` с преселектирана секция.
- **`DemoCatalogSeeder`** — 10 wellness продукта с генерирани SVG корици,
  вариации и категории за бърз преглед (`php artisan db:seed --class=DemoCatalogSeeder`).
